...
bool query_actual_position_all_channels(int address, int positions[4])
{
    ...
    for (size_t i = 0; i < responses.size() && i < 4; i++)
    {
        ...
        catch (...)
        {
            ...
            positions[i] = 0;
            std::fprintf(stderr,
                         "query_actual_position_all_channels: Invalid response for CH%zu: '%s'\n",
                         i + 1, resp.c_str());
        }
    }
...
bool query_motion_status_all_channels(int address, int statuses[4])
{
    ...
    for (size_t i = 0; i < responses.size() && i < 4; i++)
    {
        ...
        else
        {
            ...
            statuses[i] = -1;
            std::fprintf(stderr,
                         "query_motion_status_all_channels: Invalid response for CH%zu: '%s'\n",
                         i + 1, resp.c_str());
        }
    }
...
extern "C"
{
    ...
    bool pamc204_check_device(int address) { return pamc204::check_device(address); }

    bool pamc204_set_address(int new_address) { return pamc204::set_address(new_address); }
    ...
    bool pamc204_stop(int address) { return pamc204::stop(address); }

    // 位置制御API
    bool pamc204_abort_motion(int address) { return pamc204::abort_motion(address); }
